function [reward, regret, T_path, T_path2, d, timer, fixedbeams] = 2phase_ArmElim_timeframe(Env1, Env2, Time, alg, n0)
    ...
    for t = K1*K2*ex+1:Time
        ...
        if(rem(t-K1*K2*ex,n0)==1)
            ...
            if (phase1)
                % Arm Elimination
                for k2= 1:K2 
                    [b_tofix, ubi, lbi] = F_UCBi(mu_1(:,activeb'),T_1(:,activeb'),1/(t));
                ...
                %}
                arm_to_elim = false(1,K2);
                for k2= 1:K2
                   arm_to_elim = (arm_to_elim)|(ub < lb(k2));
                end
                
                activeb(arm_to_elim) = false;
                ...
                % Stop phase 1 when only one active arm left 
                if (sum(activeb,'all')==1)
                    phase1 = false;
                    fixedbeam_p2 = find(activeb==true);
                end
            else
